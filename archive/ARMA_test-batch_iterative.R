...
for (i in to_test) {
    ## ----find max_pracitcal_lag using full set for training-------------------
    max_practical_lag <- 3
    for (lag in 3:max_lag) {
        res <- validate_IRF(i, max_lag = lag,
                        train_size = train_size,
                        random_subsample_size = NULL,
                        complete = TRUE)
        if (res[3] != 1) {
            max_practical_lag <- lag
            print(paste("NEW MAX_PRACTICAL_LAG:", lag))
        }
    }
    ## plotting run
    plot_IRF(i, max_lag = max_practical_lag)
    ## validation testing with max practical lag
    res <- robust_validate_IRF(i, max_lag = lag,
                        train_size = train_size,
                        random_subsample_size = random_subsample_size,
                        folds = folds,
                        complete = TRUE)
    results[results$catchment_no == i, "mean_kge"] <- res$mean_kge
    results[results$catchment_no == i, "max_practical_lag"] <- max_practical_lag
