...
cv_results <- foreach(fold = 1:cv_folds,
    ...
    c(nrow(mod$param), mod$param[first_window, 1], mod$param[first_window, 2],                                   # nolint
      mod$param[second_window, 1], mod$param[second_window, 2],
...
colnames(cv_results) <- c("n_windows", "delta1", "sigma1", "delta2", "sigma2", "nrmse", "r2", "nse", "kge")            # nolint
...
catchment_results <- data.frame(
    ...
    criteria = criteria,
    mean_nrmse =  mean(cv_results$nrmse, na.rm = TRUE),
    mean_r2 = mean(cv_results$r2, na.rm = TRUE),
    mean_nse = mean(cv_results$nse, na.rm = TRUE),
    mean_kge = mean(cv_results$kge, na.rm = TRUE),
    n_windows = median(cv_results$n_windows, na.rm = TRUE),
    sd_windows = sd(cv_results$n_windows, na.rm = TRUE),
    mean_delta1 = mean(cv_results$delta1, na.rm = TRUE),
    mean_sigma1 = mean(cv_results$sigma1, na.rm = TRUE),
    sd_delta1 = sd(cv_results$delta1, na.rm = TRUE),
    sd_sigma1 = sd(cv_results$sigma1, na.rm = TRUE),
    mean_delta2 = mean(cv_results$delta2, na.rm = TRUE),
    mean_sigma2 = mean(cv_results$sigma2, na.rm = TRUE),
    sd_delta2 = sd(cv_results$delta2, na.rm = TRUE),
    sd_sigma2 = sd(cv_results$sigma2, na.rm = TRUE),
    median_mod_kge = median(cv_results$kge),
    median_mod_delta1 = median_mod$delta1,
    median_mod_sigma1 = median_mod$sigma1,
    median_mod_delta2 = median_mod$delta2,
    median_mod_sigma2 = median_mod$sigma2
)

print(catchment_results)

save(catchment_results, file = here("..", "data", "results", "raw",
"2w_gauss25", paste(catchment_no, "_", criteria, ".Rda", sep = "")))
}
}
