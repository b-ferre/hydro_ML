...
BIC.SWR <- function(object, ts_input, ts_output, ...){
  ...
  lik <- loglik(predict(object, newdata = ts_input),
                ts_output)
  #print(lik)
  #print(length(ts_input))

  retval <- -2 * lik + log(length(ts_input)) * 4 * dim(object)
  # print(paste("evaluating BIC. returning ", retval))
  return(retval) # -2 * loglik + k * log(n)
}
...
loglik <- function(prediction, reference){
  rss <- sum((reference - prediction)^2, na.rm = TRUE)
  n <- length(prediction)
  # print(paste("calculating loglik with rss =", rss, "n =", n))
