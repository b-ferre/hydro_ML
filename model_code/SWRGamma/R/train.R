...
decodeParam <- function(x, intercept = FALSE){
  num_win = ifelse(intercept, (length(x) - 1), length(x)) / 4
  ...
  colnames(param) <- c("delta", "shape", "rate")
  return(
    list(
      mix = mix,
      param = param
...
error <- function(ts_input, ts_output, param, mix, log){
    preds <- predict_target(ts_input, mix, param, log)
    # print(any(is.na(preds)))
    retval <- mean((preds - ts_output)^2, na.rm = TRUE)
    # print(paste("error function returning", retval))
    return(retval)
}
...
train_inc <- function(ts_input, ts_output, iter, log, param_selection = "best_bic"){
  ...
  # heuristic initialization (BUILT SPECIFICALLY FOR ENCOURAGING "DIFFERENT" WINDOWS ON 2-WINDOW RUNS)
    if(i > 1){
      ...
      shape_new <- rev(seq(from = 0.1, to = 10, by = 10/i))
      rate_new <- rep(1, i)
    }
    else{
      delta_opts = 0
