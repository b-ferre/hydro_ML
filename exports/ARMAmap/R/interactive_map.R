...
map <- function(old_data = FALSE) {
  if (old_data == FALSE) {
    ## load data
    load("./data/new_data/scores.Rda")
    load("./data/new_data/bad.Rda")
    load("./data/new_data/atlas.Rda")

  } else {
    ## load data
    load("./data/old_data/scores.Rda")
    load("./data/old_data/bad.Rda")
    load("./data/old_data/atlas.Rda")
  }
  ...
  for (j in seq_len(length(good_locs$index))) {
      i <- good_locs$index[j]
      if (old_data == FALSE) {
        data <- data.frame(sapply(read.csv(paste("./data/new_data/kernels/", i, ".csv", sep = "")), as.numeric))  # nolint
      } else {
        data <- data.frame(sapply(read.csv(paste("./data/old_data/kernels/", i, ".csv", sep = "")), as.numeric))  # nolint
      }
