...
for (i in 1:n) {
    ...
    ## delete any year that is >50% NA for either x or y
    ## (fixes issues in UK where last 4 years are completely missing)
    learnable <- function(row) {
        return(!((sum(is.na(row)) / length(row)) > 0.5))
    }
    ...
    data_dir_path <- "./research/hflm-ds_model/data/"
...
## helpful functions for post-processing
####################################################################
####################################################################
...
scored <- as.numeric(lapply(list.files("./basic_testing/results/raw_scores"), extract_gridcode))             ## nolint
n <- length(scored)
all_scores <- data.frame(gridcode <- scored, kge = rep(0, n), r2 = rep(0, n), nse = rep(0, n))                          ## nolint
for (i in seq_len(n)) {
    gridcode <- all_scores$gridcode[i]
    load(paste0("./basic_testing/results/raw_scores/grid", gridcode, "_scores.rda"))            ## nolint
    all_scores[i, c("kge", "r2", "nse")] <- scores
}
