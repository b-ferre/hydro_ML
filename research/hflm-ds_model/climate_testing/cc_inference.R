...
M <- 364
max_lag<-150
chunk_names <- list("first_chunk", "last_chunk")

#############   single catchment beta estimation func (stolen from Joe's code)   ########################## # nolint

slapdash_test <- function(gridcode, year_0, year_f) {
  ...
  ## trim data
  year_0 <- max(1, year_0)
  year_f <- min(year_f, ncol(simu_x))
  year_slice <- year_0:year_f
  simu_x <- simu_x[year_slice, ]
  Y <- Y[year_slice, ]
  ...
  return(as.numeric(pred_beta))
}

cc_test <- function(gridcode, chunk_size = 15) {
  data_dir_path <- "./data/"
  simu_x<-readRDS(file=paste0(data_dir_path, "grid",gridcode,"_rain.rds"))
  nyears <- nrow(simu_x)
  years <- rownames(simu_x)

  chunk_size <- min(chunk_size, round(nyears * 2 / 3))
  to_test <- list(c(1, chunk_size),
              c(nyears - chunk_size + 1, nyears))

  b_plots <- list()
  for (i in 1:2) {## FIXME: turn this back into seq_len(nblocks) if you want (more compute-intensive though)       # nolint
    block <- to_test[[i]]
    currently_testing <- chunk_names[[i]]

    year_0 <- years[block[1]]
    year_f <- years[block[2]]

    b_pred <- slapdash_test(gridcode, block[1], block[2])
    normed_b_pred <- b_pred / sum(b_pred, na.rm = TRUE) * 100
    save(normed_b_pred, file = paste0("./results/normalized_raw_estimates/", currently_testing, "/normed_b_pred_", year_0, "-", year_f, "_grid", gridcode, ".rda"))   # nolint

    title <- paste0(year_0, " - ", year_f)
    b_plot <- plot_bvec_better(b_pred, M, max_lag, title = title)
    b_plots <- append(b_plots, b_plot)

    jpeg(paste0("./results/plots/", currently_testing, "/beta_est_", year_0, "-", year_f, "_grid", gridcode, ".jpeg"))       # nolint
    print(b_plot)
    dev.off()

    print(paste0("block ", i, "/", 2, " finished for catchment ", gridcode))
  }
}

###################   run single catchment func on nth thru (n + 1)th catchment   ######################### # nolint
