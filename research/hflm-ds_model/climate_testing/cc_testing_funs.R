...
#plotting beta(s,t)
plot_bvec_better<-function(bvec,M,max_lag,title = "Untitled B(s, t) Estimate"){
  ...
  return(levelplot(z~x*y,mat,col.regions = colorvec,at=sort(c(seq(from=-1e-15,to=-range_n,length.out=numreds+1 ),seq(from=1e-15,to=range_p,length.out=numblues+1))),
            ...
            scales=list(x=list(cex=1.2),y=list(cex=1.2)), main = list(title, side = 1, cex = 2)))
}
