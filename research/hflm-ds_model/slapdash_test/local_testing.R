...
source("./research/hflm-ds_model/slapdash_test/testing_funs.R")
...
slapdash_test <- function(gridcode) {
  ...
  #FIXME: do this smarter using linear some kind of informed interpolation; just doing it... slapdash... for now
  for (day in 1:365) {
  ...
  q_result<-optimize_q(grpnorms = grpnorms,Nq=300,Xmat_train = Xmat,Xmat_val = Xmat,
  ...
  res_dir_path <- "./research/hflm-ds_model/slapdash_test/results/"
...
#############   single catchment R^2 testing func (stolen from Joe's code)   ##############################
## TODO:

########################   run single catchment func on all catchments   ##################################

## get a list of all catchment numbers I have formatted data for
all_files <- list.files("./research/hflm-ds_model/slapdash_test/data", pattern = "grid\\d+_rain\\.rds", full.names = FALSE)
extract_number <- function(filename) {
  return(as.integer(str_remove_all(filename, "[^\\d]")))
}
catchment_numbers <- unique(lapply(all_files, extract_number))

## filter out catchments which I have already tested
tested1 <- unique(sapply(list.files("./research/hflm-ds_model/slapdash_test/results/plots"), extract_number))
tested2 <- unique(sapply(list.files("./research/hflm-ds_model/slapdash_test/results/raw_estimates"), extract_number))
print(setdiff(tested1, tested2))
print(setdiff(tested2, tested1))
catchment_numbers <- setdiff(catchment_numbers, tested1)

## apply estimation function to every catchment I have data for
i <- 1
n <- length(catchment_numbers)
t0 <- Sys.time()
for (gridcode in catchment_numbers) {
  
  try(slapdash_test(gridcode))
  ## TODO: get this to handle errors WHILE running (some catchments are turbo-fucked like catchment 1000)

  print("##########################################################################")
  print("##########################################################################")
  print("")
  print(paste0("Finished testing catchment number ", i, " out of ", n, ". Estimated time remaining : ", (n - i) * ((Sys.time() - t0) / i)))
  print("")
  print("##########################################################################")
  print("##########################################################################")
...
## apply test set R^2 function to every catchment I have data for
## TODO:

## get a list of all catchment numbers I have formatted data for
all_data <- list.files("./research/hflm-ds_model/slapdash_test/data")
extract_number <- function(filename) {
  return(as.integer(str_remove_all(filename, "[^\\d]")))
}
catchment_numbers <- unique(lapply(all_data, extract_number))
...
indexed_gridcodes <- catchment_numbers
save(indexed_gridcodes, file = "./index.rda")

for (file in all_data) {
  if (!(extract_number(file) %in% catchment_numbers)) {
    file.remove(paste0("./research/hflm-ds_model/slapdash_test/data/", file))
  }
}

new_data <- list.files("./research/hflm-ds_model/slapdash_test/data")
print(length(new_data) / 2)
print(length(indexed_gridcodes))
