...
# fileID <- "demixing_benchmark_test1_Fig_5ac"
...
y_err_scale <- 0.5    # RMS amplitude of y errors, as a fraction of the standard deviation of the true y values
y_err_model <- list(ar=c(0.9), ma=c(-0.2, 0.2))
fileID <- "demixing_benchmark_test2_Fig_5bd"
...
bb1 <- 3*as.numeric(zz$IRF[,1])
bb2 <- 3*as.numeric(zz$IRF[,2])
bb3 <- 3*as.numeric(zz$IRF[,3])

se1 <- 3*as.numeric(zz$se[,1])
se2 <- 3*as.numeric(zz$se[,2])
se3 <- 3*as.numeric(zz$se[,3])
...
zz <- IRF(y=y, x=(x1+x2+x3)/3, m=m, verbose=verbose)

bb <- as.numeric(zz$IRF[,1])

se <- as.numeric(zz$se[,1])

benchmk <- (benchmk1+benchmk2+benchmk3)/3

# now plot results


maxval <- max(benchmk, benchmk1, benchmk2, benchmk3, bb, na.rm=TRUE) #generate upper limit for plotting
maxval <- 1.05*maxval  #and extend this a bit
minval <- -0.1*maxval  #use standardized scaling so plots can be compared

# here is the benchmark, in blue
plot(lags, benchmk, main="compare aggregated fit to benchmark", xlab="lag", ylab="kernel", xlim=c(0, max(lags)+1), ylim=c(minval, maxval), type="l", lwd=3, col="blue", pch=20, cex.main=0.6)
lines(lags, benchmk1, col="cyan")
lines(lags, benchmk2, col="cyan")
lines(lags, benchmk3, col="cyan")
# here is our result, in green
par(new=TRUE)
...
zz <- IRF(y=y, x=x1, m=m, verbose=verbose)
...
# here is the benchmark, in blue
plot(lags, benchmk, main="compare x1 fit to benchmark", xlab="lag", ylab="kernel", xlim=c(0, max(lags)+1), ylim=c(minval, maxval), type="l", lwd=3, col="blue", pch=20, cex.main=0.6)
lines(lags, benchmk1, col="cyan")
lines(lags, benchmk2, col="cyan")
lines(lags, benchmk3, col="cyan")
# here is our result, in green
...
zz <- IRF(y=y, x=x2, m=m, verbose=verbose)

bb2 <- as.numeric(zz$IRF[,1])

se2 <- as.numeric(zz$se[,1])
...
# here is the benchmark, in blue
plot(lags, benchmk, main="compare x2 fit to benchmark", xlab="lag", ylab="kernel", xlim=c(0, max(lags)+1), ylim=c(minval, maxval), type="l", lwd=3, col="blue", pch=20, cex.main=0.6)
lines(lags, benchmk1, col="cyan")
lines(lags, benchmk2, col="cyan")
lines(lags, benchmk3, col="cyan")
# here is our result, in green
...
zz <- IRF(y=y, x=x3, m=m, verbose=verbose)

bb3 <- as.numeric(zz$IRF[,1])

se3 <- as.numeric(zz$se[,1])
...
# here is the benchmark, in blue
plot(lags, benchmk, main="compare x3 fit to benchmark", xlab="lag", ylab="kernel", xlim=c(0, max(lags)+1), ylim=c(minval, maxval), type="l", lwd=3, col="blue", pch=20, cex.main=0.6)
lines(lags, benchmk1, col="cyan")
lines(lags, benchmk2, col="cyan")
lines(lags, benchmk3, col="cyan")
# here is our result, in green
...
fwrite(data.table(cbind(lags, benchmk1, benchmk2, benchmk3, benchmk, bb, se, bb1, se1, bb2, se2, bb3, se3)), file=paste0(fileID, "_averageIRF from avg and sep inputs.txt"), row.names=FALSE, sep="\t")
