...
szn_slapdash_test <- function(gridcode, season = "dorm") {
  ...
  data_dir_path <- paste0("../data/pp", season, "_data/")
    ...
    KGE <- KGE(as.numeric(Y_hat_test), as.numeric(y_test))
    NSE <- NSE(as.numeric(Y_hat_test), as.numeric(y_test))

    return(data.frame(kge = KGE, r2 = R2, nse = NSE))
}
...
for (i in i0:(i0 + 4)) {
  ...
  print("")
  print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
  print(paste0("RUNNING PREDICTIVE TESTING ALGO ON CATCHMENT NUMBER ", gridcode))
  print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
  try({
    scores <- szn_slapdash_test(gridcode, season = "dorm")
    save(scores, file = paste0("./results/dorm/raw_scores/grid", gridcode, "_scores.rda"))
    print(paste("dormant season scores for catchment", gridcode, ":"))
    print(scores)
  }, silent = FALSE)
    try({
    scores <- szn_slapdash_test(gridcode, season = "grow")
    save(scores, file = paste0("./results/grow/raw_scores/grid", gridcode, "_scores.rda"))
    print(paste("growing season scores for catchment", gridcode, ":"))
    print(scores)
  }, silent = FALSE)
}
